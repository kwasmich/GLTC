        ...
        for (b0 = 0; b0 < 32; b0++) {
            for (g0 = 0; g0 < 32; g0++) {
                for (r0 = 0; r0 < 32; r0++) {
                    ...
                    for (t1 = 0; t1 < ETC_TABLE_COUNT; t1++) {
                        for (b1 = -4; b1 < 4; b1++) {
                            if (b0 + b1 < 0 or b0 + b1 >= 32) {
                                continue;
                            }

                            for (g1 = -4; g1 < 4; g1++) {
                                if (g0 + g1 < 0 or g0 + g1 >= 32) {
                                    continue;
                                }

                                for (r1 = -4; r1 < 4; r1++) {
                                    if (r0 + r1 < 0 or r0 + r1 >= 32) {
                                        continue;
                                    }
        ...
        for (int t = 0; t < ETC_TABLE_COUNT; t++) {
            for (int p = 0; p < ETC_PALETTE_SIZE; p++) {
                ...
                if (p != 2) {
                    ...
                    ETC_UNIFORM_COLOR_LUT_NON_OPAQUE[t][p][col] = (ETCUniformColorComposition_t) {
                        col5, 0
                    };
                }
            }
...
void printInfoD(ETCBlockColor_t *in_BLOCK) {
    ...
    printf("D: (%3i %3i %3i) (%3i %3i %3i)\n", extend5to8bits(block->r), extend5to8bits(block->g), extend5to8bits(block->b), extend5to8bits(block->r + block->dR), extend5to8bits(block->g + block->dG),
           extend5to8bits(block->b + block->dB));
}
