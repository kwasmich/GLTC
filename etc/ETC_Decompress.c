...
static void _decompressEACBlockR11Signed(rgba8_t out_blockRGBA[4][4], const EACBlockR11Signed_t in_BLOCK) {
    assert(in_BLOCK.base !=
           -128);   // It is a two’s-complement value in the range [−127, 127], and where the value −128 is not allowed; however, if it should occur anyway it must be treated as −127.
