...
void computeUniformColorLUTP() {
    ...
    for (int col6 = 0; col6 < 64; col6++) {
        ...
        ETC_UNIFORM_COLOR_LUT_6[col8] = (ETCUniformColorComposition_t) {
            col6, 0
        };
    }
    ...
    for (int col7 = 0; col7 < 128; col7++) {
        ...
        ETC_UNIFORM_COLOR_LUT_7[col8] = (ETCUniformColorComposition_t) {
            col7, 0
        };
    }
...
void printInfoP(ETCBlockColor_t *in_BLOCK) {
    ...
    printf("P: (%3i %3i %3i) (%3i %3i %3i) (%3i %3i %3i)\n", extend6to8bits(block->rO), extend7to8bits(tmpG), extend6to8bits(tmpB), extend6to8bits(tmpR), extend7to8bits(block->gH),
           extend6to8bits(block->bH), extend6to8bits(block->rV), extend7to8bits(block->gV), extend6to8bits(block->bV));
}
