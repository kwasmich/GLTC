...
void computeUniformColorLUTT() {
    ...
    for (int col4 = 0; col4 < 16; col4++) {
        ...
        for (int t = 0; t < ETC_DISTANCE_TABLE_COUNT; t++) {
            int col = col8;
            ETC_UNIFORM_COLOR_LUT[t][0][col] = (ETCUniformColorComposition_t) {
                col4, 0
            };
            ETC_UNIFORM_COLOR_LUT[t][2][col] = (ETCUniformColorComposition_t) {
                col4, 0
            };
            col = clampi(col8 + ETC_DISTANCE_TABLE[t], 0, 255);
            ETC_UNIFORM_COLOR_LUT[t][1][col] = (ETCUniformColorComposition_t) {
                col4, 0
            };
            col = clampi(col8 - ETC_DISTANCE_TABLE[t], 0, 255);
            ETC_UNIFORM_COLOR_LUT[t][3][col] = (ETCUniformColorComposition_t) {
                col4, 0
            };

            col = col8;
            ETC_UNIFORM_COLOR_LUT_NON_OPAQUE[t][0][col] = (ETCUniformColorComposition_t) {
                col4, 0
            };
            col = clampi(col8 + ETC_DISTANCE_TABLE[t], 0, 255);
            ETC_UNIFORM_COLOR_LUT_NON_OPAQUE[t][1][col] = (ETCUniformColorComposition_t) {
                col4, 0
            };
            col = clampi(col8 - ETC_DISTANCE_TABLE[t], 0, 255);
            ETC_UNIFORM_COLOR_LUT_NON_OPAQUE[t][3][col] = (ETCUniformColorComposition_t) {
                col4, 0
            };
        }
    }
...
void printInfoT(ETCBlockColor_t *in_BLOCK) {
    ...
    printf("T: (%3i %3i %3i) (%3i %3i %3i) %i\n", extend4to8bits(tmpR), extend4to8bits(block->g0), extend4to8bits(block->b0), extend4to8bits(block->r1), extend4to8bits(block->g1),
           extend4to8bits(block->b1), ETC_DISTANCE_TABLE[distance]);
}
